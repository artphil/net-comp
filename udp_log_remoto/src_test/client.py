	...
	while not fim_arq or len(janela) > 0:
		...
		if linha:
			...
			if len(janela) < Wtx:
				...
				janela[msg_id]['tenta'] = 0
				janela[msg_id]['erro_e'] = 0
				janela[msg_id]['erro_r'] = 0
				janela[msg_id]['tout'] = 0
				j_lock.release()
...
def recebe():
	...
	while (not fim_arq) or len(janela) > 0:
		...
		if compare_hash(rhash, chash):
			if r_id in janela:
				j_lock.acquire()
				grava_log(	str(r_id)+', '+str(janela[r_id]['tenta'])+', '+
...
log.write(str(erro_total)+', '+str(reenvios_total-20)+', '+str(tout_total-20)+';\n')
log.close()
udp.close()
